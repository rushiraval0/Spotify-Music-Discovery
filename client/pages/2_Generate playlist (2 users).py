...
with st.form("myform", clear_on_submit=False):
    ...
    if submit:
        if uploaded_file1 is not None:
            ...
            combinedGenreDF = filter_csv(allDF,"artists",combinedArtistsList)
            ...
            filtered_df = all_filtered_df[all_filtered_df['artists'].isin(combinedArtistsList)]
            ...
            user1_songs = last_filter[last_filter['artists'].isin(user1Artists)].groupby('artists').apply(sample_up_to_3)[:10]

            # Filter and sample up to 3 songs from user2ArtistList
            user2_songs = last_filter[last_filter['artists'].isin(user2Artists)].groupby('artists').apply(sample_up_to_3)[:10]

            # Filter and sample up to 3 songs from combinedArtistList
            combined_songs = last_filter[last_filter['artists'].isin(combinedArtistsList)].groupby('artists').apply(sample_up_to_3)[:20]

            # Concatenate the filtered DataFrames
            filtered_df = pd.concat([user1_songs, user2_songs, combined_songs])
